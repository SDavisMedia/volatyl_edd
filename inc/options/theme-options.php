...
function vol_options_add_page() {
	...
	// Init EDD options
	register_setting('volatyl_edd_options', 'vol_edd_options', 'vol_options_validate');
}
...
function vol_options_do_page() {
	...
	$add_tab = array('global', 'EDD', 'content', 'hooks', 'license', 'information');
	...
	/* Tabbed - EDD Settings
	 *
	 * Display the options for Easy Digital Downloads
	 *
 	 * @since Volatyl 1.0
	 */	
	} elseif ($active_tab == 'EDD') {
		$options_edd = get_option('vol_edd_options'); 
		$vedd = volatyl_edd();
	
		echo "{$tab3}<form method=\"post\" action=\"options.php\">\n{$tab3}\t",
		settings_fields('volatyl_edd_options'),
		do_settings_sections('volatyl_edd_options'),
		
		// Start EDD options table
		"\n{$tab3}\t", __('<h3>Easy Digital Downloads Settings</h3>', 'volatyl'), "\n",
		"{$tab3}\t<div class=\"instructions radius\">\n
		{$tab3}\t\t<p>", __('You must install and activate the ', 'volatyl'), "<a href=\"http://wordpress.org/plugins/easy-digital-downloads/\" target=\"_blank\">Easy Digital Downloads</a> ", __('plugin to effectively use these options. ', 'volatyl'), "<abbr title=\"Easy Digital Downloads\">EDD</a> ", __('is a well-rounded plugin with its own ', 'volatyl'), "<a href=\"http://easydigitaldownloads.com/documentation/\" target=\"_blank\">", __('documentation', 'volatyl'), "</a>", __(' and ', 'volatyl'), "<a href=\"http://easydigitaldownloads.com/support/\" target=\"_blank\">", __('support', 'volatyl'), "</a>", __(' that you should use to customize your online store. If you are unsure whose support to visit for assistance, please visit the ', 'volatyl'), "<a href=\"http://support.volatylthemes.com/\" target=\"_blank\">Volatyl Themes ", __('Support', 'volatyl'), "</a> ", __('first.', 'volatyl'),
		"</p>\n
		{$tab3}\t</div>";
		...
		foreach ($vedd as $vd) {
			...
			$vd['th'], "\n{$tab6}", 
			$vd['td'], "\n",	
			"{$tab6}\t<input class=\"checkbox-space\" id=\"vol_edd_options[", 
			$vd['title'], "]\" name=\"vol_edd_options[", 
			$vd['title'], "]\" type=\"checkbox\" value=\"1\"";
			checked('1', $options_edd[$vd['title']], true);
			echo "/>\n{$tab6}\t<label class=\"description label-space\" for=\"vol_edd_options[", 
			$vd['title'], "]\">", 
			$vd['label'], "</label>\n{$tab6}\t\t",
		...
		echo "{$tab3}\t</table>\n
		{$tab3}\t<hr>\n{$tab3}\t<p>\n{$tab3}\t\t<input name=\"volatyl_edd_options[submit]\" id=\"submit_options_form\" type=\"submit\" class=\"button-primary\" value=\"", 
		esc_attr_e('Save EDD Settings', 'volatyl'), "\" />\n
		{$tab3}\t</p>\n
		{$tab3}</form>\n", "\t\t";
...
function vol_options_validate($input) {
	...
	$vpage = volatyl_page();
	$vedd = volatyl_edd();
	$varrays = array_merge($vgeneral, $vcontent, $vpost, $vpage, $vedd);
	foreach ($varrays as $va) { 
